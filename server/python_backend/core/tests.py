...
class NotesAPITest(TestCase):
    ...
    def test_create_note(self):
        response = self.client.post('/notes', json.dumps({
            'title': 'Test Note',
    ...
    def test_list_notes(self):
        self.test_create_note()  # Ensure there's at least one note
        response = self.client.get('/notes/')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(len(response.json()) >= 1)

    def test_get_single_note(self):
        post_response = self.client.post('/notes', json.dumps({
            'title': 'Another Note',
    ...
    def test_update_note(self):
        ...
        note_id = post_response.json()['id']
        response = self.client.put(f'/notes/{note_id}/update', json.dumps({
            'title': 'New Title'
        }), content_type='application/json')
        self.assertEqual(response.status_code, 200)
